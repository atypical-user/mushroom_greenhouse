...
#include <WiFi.h>
#include <WiFiManager.h> // https://github.com/tzapu/WiFiManager/tree/master
...
bool convertTestModeValue(String value) {
...
void connectToWiFi() {
  ...
  WiFiManagerParameter testModeTextbox("test_mode_active", "Testmode active?", "", 5);

  wm.addParameter(&awsBaseUrlTextbox);
  wm.addParameter(&testModeTextbox);
  ...
  if (!res) {
    ...
    testMode = convertTestModeValue(testModeTextbox.getValue());
    Serial.println("");
